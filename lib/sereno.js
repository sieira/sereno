...
function init(Strategy) {
  ...
  SerenoStrategy.prototype.authenticate = function(req, options) {
    ...
    this.success = function(user,info) {
      ...
      //TODO if there is not user session, send the key to the callback so it can encrypt data when needed
      success(user,info);
    }

    var test = Strategy.prototype.authenticate.call(this,req,options);
